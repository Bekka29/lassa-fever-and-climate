...
ax.barh([x for x in range(len(importance))], 100*(importance/np.sum(importance)),color='lightgreen')
...
for i, v in enumerate(values):
    ax.text(v, i, str(v)+'%', 
            color = 'Black')
fig5.savefig('Results\RNN_Permutation.pdf',dpi=900)
