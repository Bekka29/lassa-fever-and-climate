...
class GRU(nn.Module):
    def __init__(self,input_size,hidden_size,num_layers,drop_p):
        ...
        #Layer
        self.gru = nn.GRU(input_size,hidden_size,num_layers,batch_first=True,dropout=drop_p)
        self.relu = nn.ReLU()
        self.sig = nn.Sigmoid()
    ...
    def forward(self,x):
        ...
        return prediction, not_needed
    


model = GRU(input_size=hyper_params['input_size'],
...
for epoch in range(hyper_params['n_epochs']):
    with torch.no_grad():
        ...
        # Forward pass
            ...
            val_outputs,val_hidden = model(val_sequence)
    ...
    for i, (train_sequence, train_forecast) in enumerate(trainloader): 
        ...
        outputs,train_hidden = model(train_sequence) #input sequence into model
...
for i, (val_sequence, val_forecast) in enumerate(valloader): 
    ...
    val_out, val_hidden = model(val_sequence)
...
for j, (test_sequence, test_forecast) in enumerate(testloader): 
    ...
    Test_out, hidden_State = model(test_sequence)
...
torch.save(model.state_dict(), "../Weights/GRU.pt")